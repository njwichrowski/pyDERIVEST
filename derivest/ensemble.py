...
def ensemble(method, fun, x, par = None, N = 10, weights = "uniform",
    ...
    Arguments:
        ...
        eps : Positive float to serve as a precision buffer. When computing
              weighted averages of non-outlier results, any error estimates less
              than eps will be replaced with eps. A ValueError is raised if eps
              is smaller than machine precision (i.e., 1.0 + eps <= 1.0), which
              in most cases is ~2.23e-16.                      (Default: 1e-15)
        ...
        method_kwargs: Dictionary in which the keys are accepted keyword argu-
                       ...
                       [b] choosing uniformly at random from the list's elements
                       ...
                       (constant-valued arguments) in method_kwargs, since doing
                       so will merely compute an approximate derivative N times
                       at the exact same conditions.
                       
                       By default, method_kwargs varies the 'max_step' para-
                       meter according to a log-normal distribution.
        
        ob_kwargs : Dictionary of key:value pairs to be passed to outlier_bounds
    ...
    """
    args = {"method": method, "fun": fun, "par": par, "N": N, "weights": weights, "eps": eps,
            "method_kwargs": method_kwargs, "ob_kwargs": ob_kwargs, "kwargs": kwargs}
    if kwargs.pop("details", False): details = {"arguments": args}
    else: details = False
    
    # Check for valid arguments and perform set-up:
    if method in __base_methods: method = __base_methods[method] # Convert to actual function.
    elif method not in __base_methods.values():
        if isinstance(method, str): raise ValueError("Unrecognized method name: '%s'" % method)
        else: raise ValueError("Unrecognized method: %s" % method)
    if method_kwargs is None: method_kwargs = {"max_step": lognorm(2.0)}
    if method == directional_diff:
        ddiff = True
        if "d" in kwargs: d = kwargs.pop("d")
        elif "d" in method_kwargs: d = method_kwargs.pop("d")
        else: raise ValueError("Need direction 'd' for directional_diff().")
        if "normalize" in kwargs: normalize = kwargs.pop("normalize")
        else: normalize = method_kwargs.pop("d", True)
    else: ddiff = False
    if N < 3: raise ValueError("Need at least three estimates to form an ensemble.")
    if weights not in ["uniform", "error", "mean_error", "max_error", "best", "best_mean", "best_max"]:
        raise ValueError("Invalid weighting method: '%s'" % weights)
    if (1.0 + eps) <= 1.0: raise ValueError("Precision buffer value is not sufficiently positive.")
    if "deriv_order" in kwargs:
        if method == derivest: method_kwargs["deriv_order"] = kwargs.pop("deriv_order")
        else: raise ValueError("Can only specify deriv_order for derivest().")
    if par is None: par = []
    ...
    for key in method_kwargs: # Sort method arguments by type.
        ...
        try: # Do we have a scipy.stats frozen distribution?
            ...
            if isinstance(val, list): collections[key] = val # Uniform distribution over finite collection.
            elif isinstance(val, tuple): intervals[key] = val # Numeric interval w/ uniform distribution.
            else: constants[key] = val # Constant value.
    ...
    if details: iter_kw = [kw]
    if ddiff: output = directional_diff(fun, x, d, par, normalize, **kw)
    else: output = method(fun, x, par, **kw)
    try: values_ = np.zeros(list(output[0].shape) + [N])
    except AttributeError: values_ = np.zeros(N) # Use 1D array if float returned.
    values_[..., 0] = output[0]
    try: errors_ = np.zeros(list(output[1].shape) + [N])
    except AttributeError: errors_ = np.zeros(N) # Use 1D array if float returned.
    errors_[..., 0] = output[1]
    ...
    if ob_kwargs is None: ob_kwargs = {"axis": -1}
    else: ob_kwargs["axis"] = -1 # Force looking through last axis (estimates).
    ...
    if not good.any():
        ...
        good = np.logical_and(good_lo, good_hi)
        if not good.any(): raise RuntimeError("All results are numerically outliers.")
    
    # Trim the outliers from consideration:
    values = values_[..., good]
    errors = errors_[..., good]
    
    # Form the ensemble prediction via weighting method:
    if weights == "uniform": # Weight all non-outliers equally.
        deriv = np.mean(values, axis = -1)
        error = np.mean(errors, axis = -1)
    elif weights == "error": # Weight non-outliers by reciprocal error estimate, entry-wise.
        W = 1.0/np.maximum(errors, eps) # Push small errors to eps, to avoid dividing by zero.
        deriv = np.average(values, axis = -1, weights = W)
        error = np.average(errors, axis = -1, weights = W)
    elif weights == "mean_error": # Weight non-outliers by reciprocal of mean error estimate.
        W = 1.0/np.maximum(np.mean(errors, axis = all_but_last), eps)
        deriv = np.average(values, axis = -1, weights = W)
        error = np.average(errors, axis = -1, weights = W)
    elif weights == "max_error": # Weight non-outliers by reciprocal of maximum error estimate.
        ...
        deriv = values[..., idx]
    elif weights == "best_mean": # Choose estimate with smallest mean error entry.
        mean_err = np.mean(errors, axis = all_but_last)
        idx = np.argmin(mean_err)
        deriv = values[..., idx]
        error = values[..., idx]
    elif weights == "best_max": # Choose estimate with smallest maximum error entry.
