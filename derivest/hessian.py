...
def hessian(fun, x, par = None, **kwargs):
    ...
    Arguments:
        ...
        is not provided as a keyword argument, hessian() uses a default value
        of 3, rather than the ordinary default value of 2.
    ...
    zero = np.zeros(x.shape) # Zero-matrix for use with swap_elements.
    for i in range(1, x.size):
        for j in range(i):
            dij = np.zeros(terms)
            for k in range(terms):
                upup = x + replace_elements(zero, (i,j),
                        ( dfac[k]*step_size[i],  dfac[k]*step_size[j]))
                updn = x + replace_elements(zero, (i,j),
                        ( dfac[k]*step_size[i], -dfac[k]*step_size[j]))
                dnup = x + replace_elements(zero, (i,j),
                        (-dfac[k]*step_size[i],  dfac[k]*step_size[j]))
                dndn = x + replace_elements(zero, (i,j),
                        (-dfac[k]*step_size[i], -dfac[k]*step_size[j]))
                dij[k] = fun(upup) - fun(updn) - fun(dnup) + fun(dndn)
            dij /= 4.0*step_size[i]*step_size[j]*dfac**2.0
            
            # Romberg extrapolation step:
            (hess[i,j], err[i,j]) = romb_extrap(kwargs["step_ratio"],
                                                dij, [2, 4])
