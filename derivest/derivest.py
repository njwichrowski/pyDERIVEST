...
def derivest(fun, x, par = None, **kwargs):
    ...
    Arguments:
        fun : Callable object with signature fun(x, *args) -> float, with x the
              (scalar) argument, and args an optional list of parameters.
    ...
    Keyword Arguments:
        ...
        method_order : Order of the finite difference method used for estim-
                       ...
                       Higher-order methods are generally more accurate but
                       tend to be more susceptible to numerical problems. First
                       order methods are usually not recommended.  (Default: 4)
        
        romberg_terms : Number of terms to use in Romberg extrapolation. Must
                        be an integer in [0, 1, 2, 3]. Specifying a value of
                        zero disables Romberg extrapolation.       (Default: 2)
        ...
        vectorized : Boolean specifying whether fun can be evaluated at multi-
                     ple points from a single call. This minimizes the overhead
    ...
    try:
        ...
        N = 1
        number = True
    if p["fixed_step"] is None: # Use a basic sequence of steps.
        delta = p["max_step"]*p["step_ratio"]**-np.arange(26)
        num_delta = len(delta)
    else: # Fixed, user supplied absolute sequence of steps.
        num_delta = int(3 + np.ceil(p["deriv_order"]/2)
                          + p["method_order"] + p["romberg_terms"])
        if p["style"] == "central":
            num_delta -= 2
    ...
    if p["style"] == "central":
        ...
        if p["method_order"] == 2:
            if p["deriv_order"] == 1:
                ...
                fda_rule = np.array([1.0])
            elif p["deriv_order"] == 2:
                # The even transformation did all the work:
                fda_rule = np.array([2.0])
            elif p["deriv_order"] == 3:
                # Mostly odd transformation, but must remove linear term.
                fda_rule = lstsq(utils.fdamat(p["step_ratio"], 1, 2).T,
                                 np.array([[0], [1]]), rcond = None)[0].T
            elif p["deriv_order"] == 4:
                # Mostly even transformation, but must remove quadratic term.
                fda_rule = lstsq(utils.fdamat(p["step_ratio"], 2, 2).T,
                                 np.array([[0], [1]]), rcond = None)[0].T
        else: # Implies method_order == 4, since style == "central".
            if p["deriv_order"] == 1:
                # Mostly odd transformation, but must remove cubic term.
                fda_rule = lstsq(utils.fdamat(p["step_ratio"], 1, 2).T,
                                 np.array([[1], [0]]), rcond = None)[0].T
            elif p["deriv_order"] == 2:
                # Mostly even transformation, but must remove quartic term.
                fda_rule = lstsq(utils.fdamat(p["step_ratio"], 2, 2).T,
                                 np.array([[1], [0]]), rcond = None)[0].T
            elif p["deriv_order"] == 3:
                # Mostly odd transformation; remove linear, quintic terms.
                fda_rule = lstsq(utils.fdamat(p["step_ratio"], 1, 3).T,
                                 np.array([[0], [1], [0]]), rcond = None)[0].T
            elif p["deriv_order"] == 4:
                # Mostly even transformation; remove quadratic, sextic terms.
                fda_rule = lstsq(utils.fdamat(p["step_ratio"], 2, 3).T,
                                 np.array([[0], [1], [0]]), rcond = None)[0].T
    else: # Cases "forward" and "backeard" are identical, except at the end.
        if p["method_order"] == 1:
            # No odd/even transformations, but already removed constant term:
            if p["deriv_order"] == 1: fda_rule = np.array([1.0])
            else: # [2, 3, 4]
                v = np.zeros((p["deriv_order"], 1))
                v[p["deriv_order"], 0] = 1.0
                fda_rule = lstsq(utils.fdamat(p["step_ratio"],
                                              0, p["deriv_order"]).T,
                                 v, rcond = None)[0].T
        else: # p["method_order"] methods drop off the lower order terms,
              # plus terms directly above p["deriv_order"]
            nt = p["deriv_order"] + p["method_order"] - 1
            v = np.zeros((nt, 1))
            v[p["deriv_order"], 0] = 1.0
            fda_rule = lstsq(utils.fdamat(p["step_ratio"], 0, nt).T,
                             v, rcond = None)[0].T
        if p["style"] == "backward":
            fda_rule = -fda_rule # Correct sign for "backward" rule.
    if fda_rule is None:
        raise RuntimeError("Could not generate a finite difference "
                           "approximation rule.")
    ...
    if p["deriv_order"] % 2 == 0 or p["style"] != "central":
        if p["vectorized"] or N == 1:
            fx = fun(x, *par)
            if fx.shape == ():
                fx = np.reshape(fx, (1,)) # Convert scalar to length-one vector.
            if x.shape != fx.shape:
                if N > 1:
                    raise RuntimeError("fun() returned an array with an "
                                       "unexpected shape. Try specifying "
                                       "'vectorized = False'.")
                else:
                    raise RuntimeError("fun() returned an array when given "
                                       "a scalar argument.")
        else: fx = utils.loop_eval(fun, x, par) # Not vectorized, so must loop.
    else:
        fx = np.array([]) # Don't need fun(x) if central difference, odd order.
    ...
    for i in range(N):
        ...
        try:
            if p["style"] == "central": # Evaluate symmetrically around xi.
                if p["vectorized"]:
                ...
                if p["deriv_order"] in [1, 3]:
                    fev = (f_plus - f_minus)/2.0 # Odd transformation.
                else: 
                    fev = (f_plus + f_minus)/2.0 - fx[i] # Even transformation.
            elif p["style"] == "forward": # Drop off the constant only.
                if p["vectorized"]:
                    fev = fun(x + delta) - fx[i]
                else:
                    fev = np.zeros(delta.shape)
                    for j in range(num_delta):
                        fev[j] = fun(x + delta[j]) - fx[i]
            else: # Backward rule; drop off the constant only.
                if p["vectorized"]:
                    fev = fx[i] - fun(x - delta) # Swapped order to fix error.
                else:
                    fev = np.zeros(delta.shape)
                    for j in range(num_delta):
                        fev[j] = fx[i] - fun(x - delta[j]) # Ibid.
        ...
        if fev.size != num_delta:
            raise RuntimeError("fun() did not return the correct size result; "
                               "it must be vectorized.")
        
        ##### APPLY FD RULE AT EACH DELTA, SCALING AS APPROPRIATE #####
        num_estim = num_delta + 1 - num_fda - p["romberg_terms"]
        ...
        if p["style"] == "central":
            romb_expon = p["method_order"] + 2*np.arange(p["romberg_terms"])
        else:
            romb_expon = p["method_order"] + np.arange(p["romberg_terms"])
        ...
        if p["fixed_step"] is None:
            ...
            if p["deriv_order"] == 1:
                trim = np.array([0, 1, num_est - 2, num_est - 1])
            else:
                trim = np.r_[np.arange(2*(p["deriv_order"] - 1)),
                             num_est + np.arange(2*(1 - p["deriv_order"]), 0)]
            idx = np.delete(np.argsort(der_romb),
                            trim) # Get indices of non-extremal values.
    ...
    if number:
        return (float(der), float(err), float(final_delta))
    else:
        return (der, err, final_delta)
