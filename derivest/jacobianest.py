...
def jacobianest(fun, x, par = None, **kwargs):
    ...
    Arguments:
        fun : Callable object with signature fun(x, *par) -> np.ndarray, where x
              is the argument and args is an optional list of parameters.
        
        x : Location at which to compute the gradient. If x has more than one
    ...
    Example:
      ...
      ...      return np.array([v[0] + v[1]*v[2] + v[2]**3.0,
                                np.cos(v[0]**2.0 + v[1])])
      >>>  (jac, err) = jacobianest(F, np.array([1, 1, 1]))
      >>>  jac
      Out: array([[ 1.        ,  1.        ,  4.        ],
    ...
    for key in kwargs:
        if key not in p: raise TypeError("jacobianest() got an unrecognized keyword argument: %s" % key)
    ...
    if fx.size == 0: return (np.zeros((0, x.size)), np.zeros((0, x.size))) # Empty begets empty.
    (m, n) = (fx.size, x.size)
    ...
    err = np.zeros((m, n))
    for i in range(n):
        # Evaluate at each step, centered around xi. Use difference for 2nd-order estimate:
        ...
        for j in range(m): # Loop here; rombextrap with trimming would be too complicated.
            (der_romb, errors) = utils.romb_extrap(p["step_ratio"], der_est[j,:], [2, 4])
